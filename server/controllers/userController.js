  ...
  try {
    ...
    if (user.imageFileId) {
      try {
        ...
        console.log("ImageKit delete old image failed:", e.message);
      }
    }

    user.image = optimizedImageUrl;
    user.imageFileId = uploadRes.fileId;
    await user.save();

    const safeUser = await User.findById(_id).select("-password");
...
export const getCars = async (req, res) => {
  try {
    const cars = await Car.find({
      $or: [{ isAvailable: true }, { isAvaliable: true }],
    });
