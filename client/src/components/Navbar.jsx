  ...
  return (
    ...
    ${location.pathname === "/" ? "bg-light" : ""}`}>
      ...
      <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 
        ...
        borderColor px-3 rounded-full max-w-56 ${location.pathname === "/" ? "bg-light" : ""}'>
            <input type="text" className="py-1.5 w-full bg-tranparent 
            outline-none placeholder-gray-500" placeholder="Search products..."/>
            <img src={assets.search_icon} alt="search" />
        </div>

        <div className='flex max-sm:flex-col itens-start sm:items-center gap-6'>
            <button onClick={()=> navigate('/owner')} 
            className="cursor-pointer">Dashbord</button>
            <button onClick={()=> setShowLogin(true)} 
            className="cursor-pointer px-8 bg-primary 
